...
namespace bisect::bimo
{
    ...
    inline const auto size(const owning_view& s) { return s.view().size(); }

    inline const auto begin(const owning_view& s) { return s.view().begin(); }

    inline const auto end(const owning_view& s) { return s.view().end(); }
